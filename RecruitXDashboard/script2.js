...
// const GlobalFilter = (function() {
    ...
    async function init() {
        try {
            ...
            loadScript("globalFilter.js", function() {
                loadScript("chartScript.js", function() {
    ...
    function update(funnelData) {
        ...
        Object.entries(stageClasses).forEach(([key, className]) => {
            ...
            if (bar) {
                ...
                bar.dataset.count = funnelData[key];
                bar.dataset.percentage = percentage.toFixed(1);
                if (key === "offerAccepted") lastStagePercentage = percentage.toFixed(1);
            }
        });
    ...
    return {
        init: function() {
            subscribe(filteredData => {
                if (filteredData?.applications) {
                    update(calculateFunnelData(filteredData.applications));
                }
            });
        }
    };
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    try {
        ...
        populateFilters(rawData); // Populate dropdown filters
        updateCharts(); // Render charts with initial data
